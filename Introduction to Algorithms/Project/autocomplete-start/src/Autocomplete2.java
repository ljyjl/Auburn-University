   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
         ...
         for (int i = 0; i < word.length(); i++) {
            if (thisNode.mySubtreeMaxWeight < weight)
               thisNode.mySubtreeMaxWeight = weight;
                  
            char thisNodeCharacter = word.charAt(i);
               
            if (!thisNode.children.containsKey(thisNodeCharacter)) {
               thisNode.children.put(thisNodeCharacter, new Node(word.charAt(i), thisNode, weight));
               thisNode.myInfo = thisNode.myInfo + thisNodeCharacter;
            }
            
            thisNode = thisNode.children.get(thisNodeCharacter);
         }
         
         thisNode.setWord(word);
         thisNode.isWord = true;
         thisNode.myWord = word;
         
         if(thisNode.isWord && thisNode.myWeight > weight){
            thisNode.myWeight = weight;
            thisNode.setWeight(weight);
            do { 
               double maxWeight = -1;
               
               for(char c : thisNode.children.keySet()){
                  if(thisNode.children.get(c).mySubtreeMaxWeight > maxWeight)
                     maxWeight = thisNode.children.get(c).mySubtreeMaxWeight;
               }
               
               thisNode.mySubtreeMaxWeight = maxWeight;
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         for (int i = 0; i < prefix.length(); i++) {
            if (thisNode.children.containsKey(prefix.charAt(i))) {
               thisNode = thisNode.children.get(prefix.charAt(i));
            }
            else {
               return new ArrayList<String>();
            }
         ...
         PriorityQueue<Node> minimum = new PriorityQueue<Node>(k);
         pq.add(thisNode);
         
         while(!pq.isEmpty()){
            thisNode = pq.remove();
            if(thisNode.isWord && minimum.size() == k && minimum.peek().myWeight < thisNode.myWeight) {
               minimum.poll();
               minimum.add(thisNode);
            }
            else if(thisNode.isWord){
               minimum.add(thisNode);
            }
            
            for(char c : thisNode.children.keySet()) {
               pq.add(thisNode.children.get(c));
            }
         }
         
         ArrayList<String> result = new ArrayList<String>();
         
         Node[] nodes = new Node[minimum.size()];
         for(int i = 0; i < nodes.length; i++){
            nodes[i] = minimum.poll();
         }	
      
         for(int i = 0; i < nodes.length && i < k; i++){
            int index = nodes.length - 1 - i;
            result.add(nodes[index].getWord());
         }
      ...
      public String topMatch(String prefix) {
         ...
         if(thisNode.isWord && thisNode.mySubtreeMaxWeight == thisNode.getWeight()) {
            return thisNode.getWord();
         }
      
         do {
            for(char c: thisNode.children.keySet()) {
               if(thisNode.children.get(c).mySubtreeMaxWeight == thisNode.mySubtreeMaxWeight) {
                  thisNode = thisNode.children.get(c);
                  break;
               }
            }
         } while(!thisNode.isWord && thisNode.mySubtreeMaxWeight != thisNode.myWeight);
         return thisNode.myWord;
      }
      ...
      public double weightOf(String term) {
         ...
         for(int i = 0; i < term.length();i++) { 
            if(thisNode.children.containsKey(term.charAt(i))){
               thisNode = thisNode.children.get(term.charAt(i));
            }
            else{
               return 0;
            }
