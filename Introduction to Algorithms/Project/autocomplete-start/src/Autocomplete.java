  ...
  /**
   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
         // TODO: Implement add
         if (word == null)
            throw new NullPointerException("Word argument is null.");
         if (weight < 0)
            throw new IllegalArgumentException("Weight is not a proper value."); 
         ...
         for (int i = 0; i < word.length(); i++) {
            ...
            if (!myNode.children.containsKey(myChar)) {
               Node n = new Node(myChar, myNode, weight);
               myNode.children.put(myChar, n);
               myNode.myInfo += myChar;
            }
            if (myNode.mySubtreeMaxWeight < weight) 
               myNode.mySubtreeMaxWeight = weight;
            myNode = myNode.getChild(myChar);
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         for (int i = 0; i < prefix.length(); i++) {
            myNode = myNode.getChild(prefix.charAt(i));
            if (myNode == null)
               return matches;
         }
      
         PriorityQueue<Node> qC = new PriorityQueue<Node>(k, new Node.ReverseSubtreeMaxWeightComparator());
         PriorityQueue<Node> q = new PriorityQueue<Node>(k);
         qC.add(myNode);
         while(!qC.isEmpty()) {
            myNode = qC.remove();
            if (myNode.isWord && q.size() == k && q.peek().getWeight() < myNode.mySubtreeMaxWeight) {
               q.poll();
               q.add(myNode);
            }
            else if (myNode.isWord) {
               q.add(myNode);
            } 
            for (Node ch: myNode.children.values()) {
               qC.add(ch);
            }
         }
      
         Node[] ns = new Node[q.size()];
         for (int i = 0; i < ns.length; i++) {
            ns[i] = q.poll();
         }
         for (int i = ns.length - 1; i >= 0 && i >= (ns.length - k); i--) {
            matches.add(ns[i].getWord());
         }
         return matches;
      }
      ...
      public String topMatch(String prefix) {
         ...
         Node myNode = myRoot;
         for (int i = 0; i < prefix.length(); i++) {
            if (myNode.getChild(prefix.charAt(i)) != null) {
               myNode = myNode.getChild(prefix.charAt(i));
               if (myNode.isWord && myNode.getWeight() == myNode.mySubtreeMaxWeight) {
                  return myNode.getWord();
               }
            }
            else {
               return "";
         ...
         while (myNode.getWeight() != myNode.mySubtreeMaxWeight) {
            for (Node ch: myNode.children.values()) {
               if (ch.mySubtreeMaxWeight == myNode.mySubtreeMaxWeight) {
                  myNode = ch;
                  break;
               }
            }
      ...
      public double weightOf(String term) {
         ...
         for (int i = 0; i < term.length(); i++) {
            myNode = myNode.getChild(term.charAt(i));
            if (myNode != null && myNode.isWord) 
               return myNode.getWeight();
         }
         return 0.0;
      }
