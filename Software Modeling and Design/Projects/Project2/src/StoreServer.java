...
public class StoreServer {
    static String dbfile = "/Users/jiyeonlee/Desktop/Auburn_University/FALL2019/COMP3700/Activities/store.db";
        ...
        try {
            ...
            while (true) {
                ...
                if (msg.code == MessageModel.GET_PURCHASE) {
                    ...
                    PurchaseModel pp = adapter.loadPurchase(Integer.parseInt(msg.data));
                    if (pp == null) {
                        msg.code = MessageModel.OPERATION_FAILED;
                    }
                    else {
                        msg.code = MessageModel.OPERATION_OK; // load successfully!!!
                        msg.data = gson.toJson(pp);
                    }
                    out.println(gson.toJson(msg));
                }

                if (msg.code == MessageModel.PUT_PURCHASE) {
                    PurchaseModel pp = gson.fromJson(msg.data, PurchaseModel.class);
                    System.out.println("PUT command with purchase = " + pp);
                    int res = adapter.savePurchase(pp);
